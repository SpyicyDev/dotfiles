...
The fix moves the retry from "20 s at startup" to "every cheap, user-paced moment for as long as the window floats": `yabai_startup_reconcile.sh float` runs the same `unfloat_pins()` **once** — no `rule --apply`, no `arcSync()`, no `sudo` — from two signals, `space_changed` and `window_deminimized` (the one event that refreshes the cache). The common path (nothing pinned is floating) is one query + one jq and exits in ~40 ms. Properties worth knowing:
...
- **Backoff memo**: after 3 consecutive dropped un-floats a window is skipped for 10 min, except on a deminimize. A window whose cache never refreshes (never minimized) is the accepted residual — it's an upstream defect. Escape hatches: minimize/deminimize it; `yabai -m window <managed sibling id> --stack <stuck id>` (**sibling first** — the only single command that clears FLOAT *and* joins the home view regardless of the active space, `window_manager.c:1804-1816`); or restart yabai.
